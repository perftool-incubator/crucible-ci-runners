...
variable "ami_prefix" {
  type    = string
  default = "crucible-ci-fedora-43-runner-standard-arm64"
}
...
# Get the latest Fedora 43 aarch64 AMI
...
source "amazon-ebs" "fedora_runner_standard_arm64" {
  ...
  tags = {
    ...
    OS          = "Fedora 43"
    Purpose     = "GitHub Actions Runner - Standard arm64 (no K3s, no CPU isolation)"
  }
...
build {
  sources = ["source.amazon-ebs.fedora_runner_standard_arm64"]
  ...
  provisioner "shell" {
    inline = [
      ...
      # Install AWS SSM Agent for remote access (ARM64 version)
      ...
      # Pre-install GitHub Actions runner binary (ARM64 version)
      ...
      "sudo curl -sL -o actions-runner.tar.gz https://github.com/actions/runner/releases/download/v$RUNNER_VERSION/actions-runner-linux-arm64-$RUNNER_VERSION.tar.gz",
      ...
      "echo 'Standard arm64 AMI preparation complete (no K3s, no CPU isolation)!'"
    ]
  }
  ...
  post-processor "manifest" {
    output = "packer-manifest-standard-arm64.json"
    strip_path = true
  }
}
