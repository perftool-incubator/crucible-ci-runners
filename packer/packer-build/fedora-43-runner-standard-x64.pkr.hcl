...
variable "ami_prefix" {
  type    = string
  default = "crucible-ci-fedora-43-runner-standard-x64"
}
...
source "amazon-ebs" "fedora_runner_standard_x64" {
  ...
  tags = {
    ...
    OS          = "Fedora 43"
    Purpose     = "GitHub Actions Runner - Standard x64 (no K3s, no CPU isolation)"
  }
...
build {
  sources = ["source.amazon-ebs.fedora_runner_standard_x64"]
  ...
  provisioner "shell" {
    inline = [
      ...
      "echo 'Standard x64 AMI preparation complete (no K3s, no CPU isolation)!'"
    ]
  }
  ...
  post-processor "manifest" {
    output = "packer-manifest-standard-x64.json"
    strip_path = true
  }
}
